...
export function Admin() {
  ...
  const { blogs, users, payments, fetchBlogs, fetchUsers, fetchPayments, addBlog, updateBlog, deleteBlog, deleteUser, updateUserRole, updatePaymentStatus } = useAdminStore();
  ...
  const handleAddOrUpdateBlog = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingBlogId) {
      updateBlog(editingBlogId, newBlog);
    } else {
      addBlog(newBlog);
    }
                  ...
                  {paginatedBlogs.map((blog) => (
                    <div key={blog.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm hover:shadow-md transition-all group">
                      ...
                      <div className="p-6">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-3 whitespace-pre-wrap">{stripHtml(blog.content)}</p>
            ...
            >
              <div className="p-8">
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-2xl font-bold">{editingBlogId ? 'Edit Blog Post' : 'Add New Blog Post'}</h3>
                  <button 
                    onClick={() => setIsAddBlogModalOpen(false)}
                ...
                <form onSubmit={handleAddOrUpdateBlog} className="space-y-6">
                  <div>
