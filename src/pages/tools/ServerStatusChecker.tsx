...
export function ServerStatusChecker() {
  ...
  const handleAnalyze = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      setResult({ url: input, status: 'Online', responseCode: '200 OK', responseTime: Math.floor(Math.random() * 500) + 50 + 'ms' });
      setLoading(false);
    }, 1500);
  };
  ...
  return (
    <>
      <SEO title="Server Status Checker - OptiSEO Tools" description="Check the status of your server." />
      <div className="container mx-auto px-4 py-12">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Server Status Checker</h1>
          <p className="text-gray-600 dark:text-gray-400">Check the status of your server.</p>
        </div>
      ...
      <ToolDescription points={toolDescriptions['server-status-checker']} />
    </div>
    </>
  );
}
